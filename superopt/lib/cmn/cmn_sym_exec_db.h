    ...
    char const *ofile, char const *existing_file)
{
  ...
  shared_ptr<insn_db_t<cmn_insn_t, usedef_tab_entry_t>> usedef_tab = cmn_usedef_db_read(existing_file);
  ASSERT(usedef_tab);
...
shared_ptr<insn_db_t<cmn_insn_t, usedef_tab_entry_t>>
cmn_usedef_db_read(char const *filename)
{
  shared_ptr<insn_db_t<cmn_insn_t, usedef_tab_entry_t>> ret = nullptr;
  if (filename[0] != '\0') {
    ret = cmn_usedef_tab_read_from_file(filename);
  }
  if (!ret) {
    ret = make_shared<insn_db_t<cmn_insn_t, usedef_tab_entry_t>>(DEFAULT_USEDEF_TAB_HASH_SIZE);
    ASSERT(ret);
  }
  return ret;
}
