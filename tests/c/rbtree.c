...
struct node {
  int data;
  struct rbtree_elem rbtree_elem;
};

struct rbtree rt;
struct rbtree pmem_rt;

bool node_less(struct rbtree_elem const *a,
    struct rbtree_elem const *b, void *aux)
{
  assert(aux);
  struct node const* na = rbtree_entry(a, struct node, rbtree_elem);
  struct node const* nb = rbtree_entry(b, struct node, rbtree_elem);
...
main(void)
{
  rbtree_init(&rt, node_less, NULL);
  int i;
  for (i = 0; i < 10; i++) {
    struct node* n = (struct node*)malloc(sizeof(struct node));
    rbtree_insert(&rt, &n->rbtree_elem);
  }
  rbtree_init(&pmem_rt, node_less, NULL);
  for (i = 0; i < 10; i++) {
    struct node* n = (struct node*)malloc(sizeof(struct node));
    rbtree_insert(&pmem_rt, &n->rbtree_elem);
  }
  return 0;
}
