...
public:
  cg_with_dst_ml_check(cg_with_safety const &cgs) : cg_with_safety(cgs),
                                                    m_dst_memlabel_map(cgs.get_dst_tfg().get_memlabel_map())
  { }

  bool check_dst_mls() const;

  graph_memlabel_map_t get_memlabel_map() const override;
...
private:
  ...
  static predicate_set_t tfg_ec_memlabel_assumptions_negated(shared_ptr<tfg_edge_composition_t> const& ec, tfg const& t, corr_graph const& cg, pcpair const& pp, graph_memlabel_map_t const& dst_memlabel_map);
