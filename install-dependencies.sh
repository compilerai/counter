...
CLANG_VER=11
build="build-essential cmake flex bison unzip ninja-build python3-pip git"
llvm="llvm llvm-dev clang-${CLANG_VER}"
...
suggested="cscope universal-ctags atool"

apt install -y $build $llvm $libs $yices $superopt $db $fbgen $tests $compiler_explorer

apt install -y $tests $compcert $suggested
