...
typedef void rbtree_action_func(struct rbtree_elem *e, void *aux);
...
void rbtree_destroy(struct rbtree *tree, rbtree_action_func *destructor);
...
struct rbtree_elem *rbtree_find_first(struct rbtree const *tree,
    struct rbtree_elem const *elem);
struct rbtree_elem *rbtree_lower_bound(struct rbtree const *tree,
    struct rbtree_elem const *elem);
struct rbtree_elem *rbtree_upper_bound(struct rbtree const *tree,
    struct rbtree_elem const *elem);
...
void rbtree_iterate_unordered(struct rbtree *,
    void (*func)(struct rbtree_elem *, void *));

unsigned long rbtree_count(struct rbtree const *);
