...
{
  public:
  cg_suffixpath_computation(graph<pcpair, corr_graph_node, corr_graph_edge> const *g, map<pcpair, tfg_suffixpath_val_t> & in_vals)
  ...
  bool xfer_and_meet(shared_ptr<corr_graph_edge const> const &cge, tfg_suffixpath_val_t const& in, tfg_suffixpath_val_t &out) override;
};
