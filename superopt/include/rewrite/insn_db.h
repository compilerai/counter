...
public:
  ...
  void insn_db_add(I const &i, V const &v)
  {
    ...
    CPP_DBG_EXEC(INSN_MATCH, cout << __func__ << " " << __LINE__ << ": adding at index = " << index << endl);
...
dshared_ptr<insn_db_t<src_insn_t, usedef_tab_entry_t>> src_usedef_db_read(char const *filename);
dshared_ptr<insn_db_t<dst_insn_t, usedef_tab_entry_t>> dst_usedef_db_read(char const *filename);
dshared_ptr<insn_db_t<src_insn_t, ts_tab_entry_t>> src_types_db_read(char const *filename);
dshared_ptr<insn_db_t<dst_insn_t, ts_tab_entry_t>> dst_types_db_read(char const *filename);
