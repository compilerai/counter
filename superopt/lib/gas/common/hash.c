...
const char *
gas_hash_insert (struct hash_control *table, const char *key, void *val)
{
  struct hash_entry *p;
  struct hash_entry **list;
  unsigned long hash;
...
void *
gas_hash_replace (struct hash_control *table, const char *key, void *value)
{
  struct hash_entry *p;
  void *ret;
...
void *
gas_hash_find (struct hash_control *table, const char *key)
{
  struct hash_entry *p;
...
void *
gas_hash_delete (struct hash_control *table, const char *key, int freeme)
{
  struct hash_entry *p;
  struct hash_entry **list;
...
void
gas_hash_print_statistics (FILE *f ATTRIBUTE_UNUSED,
...
main ()
{
  ...
  for (;;)
	...
	case 'd':
	  ...
	  break;
	case 'f':
	  p = gas_hash_find (h, name = what ("symbol"));
	...
	case 'h':
	  ...
	  break;
	case 'i':
	  p = gas_hash_insert (h, name = what ("symbol"), value = what ("value"));
	  if (p)
	    {
	...
	case 'q':
	  exit (EXIT_SUCCESS);
	case 'r':
	  p = gas_hash_replace (h, name = what ("symbol"), value = what ("value"));
	...
	case 's':
	  ...
	  break;
	case 'x':
	  p = gas_hash_delete (h, name = what ("symbol"));
	  printf ("old value was \"%s\"\n", p ? p : "{}");
	  break;
	default:
