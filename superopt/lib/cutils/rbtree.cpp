...
static struct rbtree_elem *
find_last_equal_entry(struct rbtree const *tree,
		struct rbtree_elem const *elem, struct rbtree_elem *root)
{
	while (1) {
		struct rbtree_elem *succ;
		succ = successor(root);
		if (succ == nil || tree->less(elem, succ, tree->aux)) {
			break;
		}
		ASSERT(   !tree->less(elem, succ, tree->aux)
				   && !tree->less(succ, elem, tree->aux));
		root = succ;
	}
	ASSERT(root != nil);
	ASSERT(   !tree->less(elem, root, tree->aux)
...
/* Find the last element that is less than or equal to ELEM. */
struct rbtree_elem *
rbtree_lower_bound(struct rbtree const *tree, struct rbtree_elem const *elem)
{
	struct rbtree_elem *root = tree->root, *ret = NULL; 
	bool root_is_less, root_is_greater;
	...
	while (root != nil) {
		...
		if (root_is_greater) {
			...
			if (root_is_less) {
				...
				ret = find_last_equal_entry(tree, elem, root);
        return ret;
			}
		}
	}
	ASSERT(!ret || tree->less(ret, elem, tree->aux));
	return ret;
}

/* Find the first element that is greater than ELEM. */
struct rbtree_elem *
rbtree_upper_bound(struct rbtree const *tree, struct rbtree_elem const *elem)
{
	struct rbtree_elem *root = tree->root, *ret = NULL;
	bool /*root_is_less, */root_is_greater;
	...
	ASSERT(!ret || tree->less(elem, ret, tree->aux));
	return ret;
}
...
inorder(struct rbtree_elem const *root, rbtree_print_func *print, void *aux)
{
  ...
  printf("%p[%s] (left %p, right %p, parent %p) ", root,
...
void
rbtree_inorder(struct rbtree const *tree, rbtree_print_func *print)
{
  inorder(tree->root, print, tree->aux);
}
...
static void
rbtree_elem_iterate_unordered(struct rbtree_elem *root,
    void (*func)(struct rbtree_elem *, void *), void *aux)
{
  if (root == nil) {
    return;
  }

  rbtree_elem_iterate_unordered(root->left, func, aux);
  rbtree_elem_iterate_unordered(root->right, func, aux);

  (*func)(root, aux);
}

void
rbtree_iterate_unordered(struct rbtree *tree,
    void (*func)(struct rbtree_elem *, void *))
{
  rbtree_elem_iterate_unordered(tree->root, func, tree->aux);
}

static void
rbtree_clear(struct rbtree_elem *root, rbtree_action_func *destructor, void *aux)
{
  if (root == nil) {
    return;
  }
  rbtree_clear(root->left, destructor, aux);
  rbtree_clear(root->right, destructor, aux);
  (*destructor)(root, aux);
}

void
rbtree_destroy(struct rbtree *tree, rbtree_action_func *destructor) 
{
  if (destructor != NULL) {
    rbtree_clear(tree->root, destructor, tree->aux);
  }
}

unsigned long
rbtree_count(struct rbtree const *tree)
{
  struct rbtree_elem *iter;
  unsigned long ret;

  ret = 0;
  for (iter = rbtree_begin(tree); iter != NULL; iter = rbtree_next(iter)) {
    ret++;
  }
  return ret;
}
