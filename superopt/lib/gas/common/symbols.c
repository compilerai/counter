...
symbol_find_exact_noref (const char *name, int noref)
{
  ...
  locsym = (struct local_symbol *) gas_hash_find (local_hash, name);
  if (locsym != NULL)
    return (symbolS *) locsym;

  sym = ((symbolS *) gas_hash_find (sy_hash, name));
...
symbol_begin (void)
{
  ...
  sy_hash = gas_hash_new ();
  local_hash = gas_hash_new ();

  //printf("%s() %d\n", __func__, __LINE__);
  memset ((char *) (&abs_symbol), '\0', sizeof (abs_symbol));
#if defined (EMIT_SECTION_SYMBOLS) || !defined (RELOC_REQUIRES_SYMBOL)
  //printf("%s() %d\n", __func__, __LINE__);
  abs_symbol.bsym = 0; //bfd_abs_section_ptr->symbol;
  //printf("%s() %d\n", __func__, __LINE__);
#endif
  //printf("%s() %d\n", __func__, __LINE__);
...
{
  gas_hash_print_statistics (file, "symbol table", sy_hash);
  gas_hash_print_statistics (file, "mini local symbol table", local_hash);
