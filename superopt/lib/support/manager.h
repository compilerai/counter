...
public:
  manager()
  {
    ...
    eqspace::stats::get().register_manager(demangled, *this);
    free(demangled);
  }
  ~manager()
  {
    int status;
    char * demangled = abi::__cxa_demangle(typeid(T).name(),0,0,&status);
    eqspace::stats::get().deregister_manager(demangled);
    free(demangled);
  }
  std::shared_ptr<T> register_object(T const &given, unsigned suggested_id = 0)
  {
    ...
    if (it == m_object_map.end()) {
      ...
      //std::cout << __func__ << " " << __LINE__ << ": returning " << new_elem.get() << std::endl;
