...
#define ASSERTCHECK(...) ASSUME(cond)
#define FFLUSH(...)
#define DBGn(...)
#define _DBGn(...)
#define DBG_EXECn(...)
...
#define MSG(s) do { printf("<MSG>%s: %s</MSG>\n", get_timestamp(gtbuf, sizeof gtbuf), s); if (eqspace::g_xml_output_stream.is_open()) eqspace::g_xml_output_stream << "<MSG>" << s << "</MSG>"; eqspace::g_xml_output_stream.flush(); CPP_DBG_EXEC(COUNTERS_ENABLE, stats::get().print_smt_solver_counters()); } while (0)

#define MSGS(s) do { ostringstream os; os << s; MSG(os.str().c_str()); } while (0)

#define ERR(s) do { printf("<ERR>%s</ERR>\n", s); if (eqspace::g_xml_output_stream.is_open()) eqspace::g_xml_output_stream << "<ERR>" << s << "</ERR>"; eqspace::g_xml_output_stream.flush(); } while (0)

extern char progress_flag;
#ifdef __cplusplus
extern thread_local bool g_expect_no_deterministic_malloc_calls;
#endif
...
#define PRINT_PROGRESS(f, ...) do { \
  if (progress_flag) { \
    ...
    fprintf(stderr, "%ld:%02ld :: mem tot %.3fG rss %.3fG :: num_cg_constructions %lld num_cg_destructions %lld active %lld :: aliasing_constraints constructions %lld destructions %lld active %lld :: memlabel_assertions constructions %lld destructions %lld active %lld :: invariant_state constructions %lld destructions %lld active %lld :: smallest_point_cover constructions %lld destructions %lld active %lld :: predicate constructions %lld destructions %lld active %lld :: point_set constructions %lld destructions %lld active %lld :: point_val constructions %lld destructions %lld active %lld :: array_constant constructions %lld destructions %lld active %lld :: counter_example_t constructions %lld destructions %lld active %lld :: graphce_t constructions %lld destructions %lld active %lld :: " f, \
        (long int)((tv.tv_sec-start_time.tv_sec)/60), \
        ...
        stats_num_smallest_point_cover_constructions, stats_num_smallest_point_cover_destructions, stats_num_smallest_point_cover_constructions - stats_num_smallest_point_cover_destructions, \
...
bool belongs_to_peep_program_slice(src_ulong pc);
int print_debug_info_in(src_ulong pc);
int print_debug_info_out(src_ulong pc);

void print_alloc_id(char const* prefix, unsigned id, void *ptr);
void scan_alloc_id(char const* prefix, unsigned id, void *ptr);
...
extern long long stats_num_smallest_point_cover_constructions, stats_num_smallest_point_cover_destructions;
...
DBG_DECL(LOCKSTEP);
DBG_DECL(CLANG_DRIVER);
DBG_DECL(SMT_SOLVERS);
DBG_DECL(COUNTERS_ENABLE);
DBG_DECL(QD_HELPER_PROCESS);
