...
public:
  ...
  serpar_composition_node_t(T_ATOM const &e) : m_type(atom), m_atom(make_shared<T_ATOM const>(e))
  {
  }
  ...
  list<shared_ptr<T_ATOM const>> get_constituent_atoms() const
  {
    list<shared_ptr<T_ATOM const>> ret;
    if (m_type == serpar_composition_node_t::atom) {
      ret.push_back(m_atom);
      return ret;
    }
    list<shared_ptr<T_ATOM const>> ret_a = m_a->get_constituent_atoms();
    list<shared_ptr<T_ATOM const>> ret_b = m_b->get_constituent_atoms();
    for (auto b : ret_b) {
      ret_a.push_back(b);
    }
    return ret_a;
  ...
  shared_ptr<T_ATOM const> get_atom() const { ASSERT(m_atom); return m_atom; }
  ...
  template<typename T_RET>
  T_RET visit_nodes(std::function<T_RET (shared_ptr<T_ATOM const> const &)> f, std::function<T_RET (type, T_RET const &, T_RET const &)> fold_f) const
  {
    if (m_type == atom) {
      return f(m_atom);
    } else {
  ...
  void visit_nodes_const(std::function<void (shared_ptr<T_ATOM const> const &)> f) const
  {
    if (m_type == atom) {
      f(m_atom);
    } else {
  ...
  template<typename T_RET>
  T_RET visit_nodes_postorder_series(std::function<T_RET (shared_ptr<T_ATOM const> const &, T_RET const &)> fold_atom_f,
                                     std::function<T_RET (T_RET const &, T_RET const &)> fold_parallel_f,
                                     T_RET const &postorder_val) const
  {
    if (m_type == atom) {
      return fold_atom_f(m_atom, postorder_val);
    } else if (m_type == series) {
      T_RET ret_b = m_b->visit_nodes_postorder_series(fold_atom_f, fold_parallel_f, postorder_val);
      return m_a->visit_nodes_postorder_series(fold_atom_f, fold_parallel_f, ret_b);
    } else if (m_type == parallel) {
      T_RET ret_a = m_a->visit_nodes_postorder_series(fold_atom_f, fold_parallel_f, postorder_val);
      T_RET ret_b = m_b->visit_nodes_postorder_series(fold_atom_f, fold_parallel_f, postorder_val);
      return fold_parallel_f(ret_a, ret_b);
    } else NOT_REACHED();
  }
  template<typename T_RET>
  T_RET visit_nodes_preorder_series(std::function<T_RET (shared_ptr<T_ATOM const> const &, T_RET const &)> fold_atom_f,
                                     std::function<T_RET (T_RET const &, T_RET const &)> fold_parallel_f,
                                     T_RET const &preorder_val) const
  {
    if (m_type == atom) {
      ...
      T_RET ret_a = m_a->visit_nodes_preorder_series(fold_atom_f, fold_parallel_f, preorder_val);
      return m_b->visit_nodes_preorder_series(fold_atom_f, fold_parallel_f, ret_a);
    } else if (m_type == parallel) {
      T_RET ret_a = m_a->visit_nodes_preorder_series(fold_atom_f, fold_parallel_f, preorder_val);
      T_RET ret_b = m_b->visit_nodes_preorder_series(fold_atom_f, fold_parallel_f, preorder_val);
      return fold_parallel_f(ret_a, ret_b);
    } else NOT_REACHED();
  }
...
private:
  ...
  shared_ptr<T_ATOM const> const m_atom;
