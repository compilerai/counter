...
//  shared_ptr<tfg_edge_composition_t> find_implied_path(shared_ptr<corr_graph const> cg_parent, shared_ptr<corr_graph> cg, pcpair const &pp, shared_ptr<tfg_edge_composition_t> const &src_path, shared_ptr<tfg_edge_composition_t> const &dst_edge, bool &implied_check_status/*, predicate &edge_cond_pred*/) const;
  ...
  shared_ptr<corr_graph_edge const> create_new_corr_graph_edge(dshared_ptr<corr_graph> cg, pcpair const &from_pc, pcpair const &to_pc, shared_ptr<tfg_full_pathset_t const> src_edge, shared_ptr<tfg_full_pathset_t const> const &dst_ec, list<dshared_ptr<corr_graph_node>>* added_nodes = nullptr) const;
  dshared_ptr<corr_graph_edge const> add_corr_graph_edge(dshared_ptr<corr_graph> cg, dshared_ptr<corr_graph_edge const> const& e) const;
  void remove_corr_graph_edge(corr_graph& cg, dshared_ptr<corr_graph_edge const> edge);

  graph_edge_composition_ref<pcpair,corr_graph_edge> corr_graph_create_and_add_cg_edge_composition_using_src_and_dst_fp(dshared_ptr<cg_with_asm_annotation> cg_new, pcpair const& cg_from_pc, pcpair const& cg_to_pc, shared_ptr<tfg_full_pathset_t const> const& src_path, shared_ptr<tfg_full_pathset_t const> const& dst_path) const;
  graph_edge_composition_ref<pcpair,corr_graph_edge> corr_graph_try_creating_edges_by_pairing_allocdeallocs_together(dshared_ptr<cg_with_asm_annotation> cg_new, pcpair const& cg_from_pc, pcpair const& cg_to_pc, shared_ptr<tfg_full_pathset_t const> const& src_path, shared_ptr<tfg_full_pathset_t const> const& dst_path, map<pair<alloc_dealloc_pc_t,alloc_dealloc_t>,pair<pc,list<pc>>> const& common_allocdeallocs) const;
  graph_edge_composition_ref<pcpair,corr_graph_edge> corr_graph_try_adding_edges_by_pairing_src_dst_edges_based_on_to_pc(dshared_ptr<cg_with_asm_annotation> cg_new, pcpair const& cg_from_pc, pcpair const& cg_end_pc, function<map<pc_label_t,list<shared_ptr<tfg_full_pathset_t const>>>(pc const&)> get_relevant_src_paths_from, function<map<pc_label_t,list<shared_ptr<tfg_full_pathset_t const>>>(pc const&)> get_relevant_dst_paths_from, list<dshared_ptr<corr_graph_edge const>>& to_be_added_edges, list<dshared_ptr<corr_graph_node>>& added_nodes) const;
