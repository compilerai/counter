...
{
private:
  mutable set<expr_group_t::expr_idx_t> m_masked_exprs;
public:
...
protected:
  ...
  {
    if (this->m_masked_exprs.count(idx)) {
      return true;
    }
    if (this->m_out_of_bound_exprs.count(idx)) {
      return true;
    }
  ...
  virtual bool recompute_preds_for_points(bool last_proof_success, unordered_set<shared_ptr<T_PRED const>>& preds) const override
  {
    ...
    if (this->m_timeout_info.timeout_occurred()) {
      ...
      for (auto const& timeout_pred : timeout_preds) {
        ...
        auto timeout_expr_idx = exprs_to_be_correlated->expr_group_get_index_for_expr(timeout_expr);
        ASSERT(timeout_expr_idx != expr_group_t::EXPR_IDX_INVALID);
        m_masked_exprs.insert(timeout_expr_idx);
      }
    }
