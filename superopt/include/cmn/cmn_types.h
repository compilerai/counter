...
dshared_ptr<insn_db_t<cmn_insn_t, ts_tab_entry_t>>
cmn_ts_tab_read_from_file(char const *file)
{
  ...
  if (!fp) {
    ...
    return dshared_ptr<insn_db_t<cmn_insn_t, ts_tab_entry_t>>::dshared_nullptr();
  }
  ASSERT(fp);

  dshared_ptr<insn_db_t<cmn_insn_t, ts_tab_entry_t>> ts_tab = make_dshared<insn_db_t<cmn_insn_t, ts_tab_entry_t>>(DEFAULT_TS_TAB_HASH_SIZE);
  ASSERT(ts_tab);
  ...
  while (   (fs = fscanf(fp, "%[^\n;]%c", line, &delim)) != EOF
    ...
    ASSERT(sa != EOF);
    linenum += sa;
    string assembly_section = string(section_buffer);
    ...
    if (cmn_insn_t::uses_serialization()) {
      ...
      DBG(TYPESTATE, "assembly_section:\n%s\ncmn_iseq:\n%s\n", assembly_section.c_str(),
    ...
    ts_tab->insn_db_add(cmn_iseq[0], new_entry);
    ts_tab_num_entries++;
  }
  ASSERT(fs == EOF);
...
static dshared_ptr<insn_db_t<cmn_insn_t, ts_tab_entry_t>>
cmn_ts_tab_init(/*, bool src*/)
{
  autostop_timer func_timer(__func__);
  char const *file = SUPEROPTDBS_DIR cmn_prefix ".insn.types.preprocessed";
  return cmn_ts_tab_read_from_file(file);
}
