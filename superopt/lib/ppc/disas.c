...
{ "bdzflrl-",XLO(19,BODZF,16,1), XLBOBB_MASK, PPC,	{ BI } },
{ "bdzflrl+",XLO(19,BODZFP,16,1), XLBOBB_MASK, PPC,	{ BI } }, sorav */
...
{ "bfctrl-", XLO(19,BOF,528,1), XLBOBB_MASK, PPC,	{ BI } },
{ "bfctrl+", XLO(19,BOFP,528,1), XLBOBB_MASK, PPC,	{ BI } }, sorav */
...
static int
snprint_insn_powerpc (char *buf, size_t size, disassemble_info *info,
    uint32_t insn, unsigned memaddr, int dialect);

/* Print a big endian PowerPC instruction.  For convenience, also
   disassemble instructions supported by the Motorola PowerPC 601.  */

static int
__snprint_insn_ppc (char *obuf, size_t size, bfd_vma pc, bfd_byte *ibuf,
    disassemble_info *info)
{
    ...
    if (info->mach == bfd_mach_ppc64) {
        return snprint_insn_powerpc (obuf, size, info, opc, pc,
                                    PPC | B64);
    } else {
        return snprint_insn_powerpc (obuf, size, info, opc, pc,
                                    PPC | B32 | M601);
    ...
    uint32_t insn, unsigned memaddr, int dialect)
{
  ...
  for (opcode = powerpc_opcodes; opcode < opcode_end; opcode++) {
    ...
    ptr += (*info->snprintf_func)(ptr, end - ptr, "%s", opcode->name);
    ...
    for (opindex = opcode->operands; *opindex != 0; opindex++) {
      ...
      if ((operand->flags & PPC_OPERAND_GPR) != 0)
        //ptr += (*info->snprintf_func)(ptr, end - ptr, "r%d", value);
        ptr += (*info->snprintf_func)(ptr, end - ptr, "%d", value);
      else if ((operand->flags & PPC_OPERAND_FPR) != 0)
        ptr += (*info->snprintf_func)(ptr, end - ptr, "f%d", value);
      else if ((operand->flags & PPC_OPERAND_RELATIVE) != 0)
        ptr += (*info->snprintf_func)(ptr, end - ptr, "%08X", memaddr + value);
      else if ((operand->flags & PPC_OPERAND_ABSOLUTE) != 0)
        ptr += (*info->snprintf_func)(ptr, end - ptr, "%08X", value & 0xffffffff);
      else if ((operand->flags & PPC_OPERAND_CR) == 0
          || (dialect & PPC_OPCODE_PPC) == 0)
        ptr += (*info->snprintf_func)(ptr, end - ptr, "0x%x", value);
      else {
        if (operand->bits == 3)
          ptr += (*info->snprintf_func)(ptr, end - ptr, "cr%d", value);
        else {
          ...
          cr = value >> 2;
          if (cr != 0)
            ptr += (*info->snprintf_func)(ptr, end - ptr, "4*cr%d", cr);
          cc = value & 3;
          if (cc != 0)
          {
            if (cr != 0) {
            ...
            ptr += (*info->snprintf_func)(ptr, end - ptr, "%s", cbnames[cc]);
          }
        }
      }
  ...
  ptr += (*info->snprintf_func)(ptr, end - ptr, ".long 0x%x", insn);
    ...
    uint8_t *code, target_ulong size, int flags)
{
  ...
  INIT_DISASSEMBLE_INFO(disasm_info, NULL, snprintf);
  ...
  for (pc = nip; pc < nip + size; pc += count) {
    if (flags == -1) {
      ptr += snprintf(ptr, end - ptr, "0x" SRC_FMT_lx ":  ", pc);
    }
    count = __snprint_insn_ppc(ptr, end - ptr, (bfd_vma)pc, code, &disasm_info);
    ptr += strlen(ptr);
    if (count < 0)
      break;
    ...
    if (flags == -1) /*sorav*/ {
      ...
      memset_init(&memuse);
      memset_init(&memdef);
      //printf("calling get_ppc_usedef_regs for 0x%x: 0x%x\n", pc, opcode);
      get_ppc_usedef(code, &use, &def, &memuse, &memdef);
      ptr += snprintf (ptr, end - ptr, "\t\t\t(use:%s, def:%s)",
          regset_to_string(&use, buf1, sizeof buf1),
          regset_to_string(&def, buf2, sizeof buf2));
      memset_free(&memuse);
      memset_free(&memdef);
    }
#endif
