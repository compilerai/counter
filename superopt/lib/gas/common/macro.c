...
do_formals (macro_entry *macro, size_t idx, sb *in)
{
  ...
  while (idx < in->len)
      ...
      if (idx < in->len && in->ptr[idx] == '=')
	{
      ...
      if (! gas_hash_find (macro->formal_hash, name))
	      ...
	      const char **namep)
{
  ...
  if (gas_hash_find (macro_hash, macro->name))
	    ...
	    int kind, sb *out, int copyifnotthere)
{
  ...
  if (macro_strip_at
    ...
    ptr = (formal_entry *) gas_hash_find (formal_hash, sb_terminate (t));
  if (ptr)
    {
      if (ptr->actual.len)
	{
		   ...
		   || (src > 0 && in->ptr[src - 1] == '@')))
	{
	  if (! macro
	      ...
	      while (in->ptr[src] != '\n')
		{
		  ...
		  if (! gas_hash_find (formal_hash, name))
		    {
		      static int loccnt;
		      char buf[20];
  ...
  while (loclist != NULL)
    {
      ...
      gas_hash_delete (formal_hash, name, f == NULL);
      del_formal (loclist);
      loclist = f;
    }
...
macro_expand (size_t idx, sb *in, macro_entry *m, sb *out)
{
  ...
  while (idx < in->len)
      ...
      if (scan < in->len && !macro_alternate && in->ptr[scan] == '=')
	{
	  ...
	  ptr = (formal_entry *) gas_hash_find (m->formal_hash, sb_terminate (&t));
	  if (!ptr)
	    {
	     ...
	     const char **error, macro_entry **info)
{
  ...
  macro = (macro_entry *) gas_hash_find (macro_hash, copy);
...
delete_macro (const char *name)
{
  ...
  if ((macro = (macro_entry *) gas_hash_find (macro_hash, copy)) != NULL)
    {
