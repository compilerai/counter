...
public:
  ...
  uint8_t get_loc() const override
  {
    return TMAP_LOC_SYMBOL;
  }
  dst_ulong get_regnum() const override
  {
    return m_addr;
  }
  reg_identifier_t const &get_reg_id() const override
  {
    NOT_REACHED();
  }
  void update_regnum(dst_ulong regnum) override
  {
    m_addr = regnum;
  }
  void update_reg_id(reg_identifier_t const &reg_id) override
  {
    NOT_REACHED();
  }
...
{
private:
  dshared_ptr<transmap_loc_object_t> m_ptr;

  static dshared_ptr<transmap_loc_object_t> get_new_ptr_from_obj(transmap_loc_object_t const &obj)
  {
    transmap_loc_symbol_t const *sym = dynamic_cast<transmap_loc_symbol_t const *>(&obj);
    if (sym) {
      return make_dshared<transmap_loc_symbol_t>(*sym);
    }
    transmap_loc_exreg_t const *exreg = dynamic_cast<transmap_loc_exreg_t const *>(&obj);
    if (exreg) {
      return make_dshared<transmap_loc_exreg_t>(*exreg);
    }
    NOT_REACHED();
  }
public:
  ...
  {
    dshared_ptr<transmap_loc_symbol_t const> sym = dynamic_pointer_cast<transmap_loc_symbol_t const>(m_ptr);
    if (sym) {
      return sym->transmap_loc_symbol_to_string();
    }
    dshared_ptr<transmap_loc_exreg_t const> exreg = dynamic_pointer_cast<transmap_loc_exreg_t const>(m_ptr);
    if (exreg) {
  ...
  {
    dshared_ptr<transmap_loc_exreg_t> exreg = dynamic_pointer_cast<transmap_loc_exreg_t>(m_ptr);
    ASSERT(exreg);
