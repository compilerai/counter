...
convert_valtag_to_expr(context *ctx, valtag_t const &vt, sort_ref const &s)
{
  ...
  if (vt.tag == tag_const) {
    ...
    size_t len = imm2str(vt.val, vt.tag, vt.mod.imm.modifier, vt.mod.imm.constant_val, vt.mod.imm.constant_tag, vt.mod.imm.sconstants, vt.mod.imm.exvreg_group, nullptr, 0, I386_AS_MODE_32, as1, sizeof as1);
    string name = as1;
    ...
    long max_num_out_var_ops, imm_vt_map_t const *st_map, context *ctx)
{
  ...
  ASSERT(in->is_const() || cs.is_symbol(in) || cs.expr_is_local_id(in));
  valtag_t vt;
  if (in->is_const()) {
    vt.tag = tag_const;
  ...
  /*} else if (cs.expr_represents_llvm_undef(in)) {
    vt.tag = tag_const;
    vt.val = 0;*/
  } else if (cs.expr_is_local_id(in)) {
    vt.tag = tag_const;
    local_id_t local_id = cs.expr_get_local_id(in);
    ASSERT(local_id >= 0 && local_id < NUM_CANON_SYMBOLS);
    vt.val = RANDOM_CONSTANT_FOR_LOCAL_ID(local_id); //use some random constant that ensures that each symbol is assigned a different canonical constant
  } else NOT_REACHED();
...
tfg_edges_equal(shared_ptr<tfg_edge const> const &e1, shared_ptr<tfg_edge const> const &e2)
{
  ...
  if (e1->get_to_state().to_string_for_eq() != e2->get_to_state().to_string_for_eq()) {
    return false;
  }
  return true;
}
...
expr_matches_template_visitor::visit_recursive(expr_ref const &a, expr_ref const &b, vector<src_iseq_match_renamings_t> const &input_renamings)
{
  ...
  if (   a->is_var()
      ...
      && (b->is_const() || m_cs.is_symbol(b) || expr_is_canonical_constant(b, NULL) || m_cs.expr_represents_llvm_undef(b) || m_cs.expr_is_local_id(b))
    ...
    if (b->is_const()) {
      ...
      bvt.tag = tag_const;
      bvt.val = b_val;
    } else if (m_cs.expr_is_local_id(b)) {
      local_id_t local_id = m_cs.expr_get_local_id(b);
...
"}}}\n";
  ...
  long ret = src_iseq_from_string(NULL, insns, insns_size, str, I386_AS_MODE_32);
...
tfg_get_first_insn_num_from_pc(const void *tfg_in, pc const &p)
{
  ...
  list<shared_ptr<tfg_edge const>> const &es = t->get_edges_ordered();
  int insn_num = 0;
  pc prev_from_pc;
  for (auto iter = es.begin(); iter != es.end(); iter++) {
    shared_ptr<tfg_edge const> const &e = *iter;
    if (e->get_from_pc() == p) {
      return insn_num;
    }
...
bool etfg_insn_is_function_return(etfg_insn_t const *insn)
{
  ...
  state const &to_state = te->get_to_state();
  string k;
  if ((k = to_state.has_expr_substr(LLVM_STATE_INDIR_TARGET_KEY_ID)) == "") {
    //cout << __func__ << " " << __LINE__ << ":\n";
    return false;
  }
...
expr_strict_canonicalize_regs_visitor::visit(expr_ref const &e)
{
  ...
  if (e->is_var()) {
    if (e->is_array_sort() || e->is_memlabel_sort() || e->is_function_sort()) {
      m_map[e->get_id()] = e;
      return;
    }
    //cout << __func__ << " " << __LINE__ << ": var = " << expr_string(e) << ": is arg const = " << m_cs.expr_is_argument_constant(e) << endl;
    if (   expr_is_consts_struct_constant(e, m_cs) == -1
    ...
    fixed_reg_mapping_t const &fixed_reg_mapping)
{
  ...
  expr_ref econd = e->get_cond();
  //expr_ref indir_tgt = e->get_indir_tgt();
  state const &strict_start_state = t_strict->get_start_state();
    ...
    if (to_pc.is_exit()) {
      if (!(*iter)->edge_is_indirect_branch()) {
        if ((*iter)->get_to_state().get_value_expr_map_ref().size() != 0) {
          ...
          cout << __func__ << " " << __LINE__ << ": (*iter)->get_to_state() = " << (*iter)->get_to_state().to_string() << endl;
        }
...
"nop:1\n"
"}}}\n";
  long ret = src_iseq_from_string(NULL, insns, insns_size, etfg_nop_insn_str, I386_AS_MODE_32);
  ASSERT(ret <= insns_size);
  ASSERT(ret == 1);
  return ret;
}
...
{
  tfg *ret = new tfg_llvm_t(ETFG_ISEQ_TFG_NAME, ctx/*, etfg_canonical_start_state(ctx)*/);
  ret->set_start_state(etfg_canonical_start_state(ctx));
  for (auto e : edges) {
    pc from_pc = e.get_tfg_edge()->get_from_pc();
    pc to_pc = e.get_tfg_edge()->get_to_pc();
    shared_ptr<tfg_node> from_node = ret->find_node(from_pc);
    shared_ptr<tfg_node> to_node = ret->find_node(to_pc);
    if (!from_node) {
      from_node = make_shared<tfg_node>(from_pc);
      ret->add_node(from_node);
    }
    if (!to_node) {
      to_node = make_shared<tfg_node>(to_pc);
...
etfg_insn_edge_read(istream &in, string line, context *ctx)
{
  ...
  line = tfg_edge::graph_edge_from_stream(in, line, prefix/*, start_state*/, ctx, e);
...
{
  stringstream ss;
  ss << etfg_canonical_start_state(g_ctx).to_string_for_eq();
...
{
  string k;
  if ((k = state_to.has_expr_substr(LLVM_STATE_INDIR_TARGET_KEY_ID)) == "") {
    return false;
  }
...
#define MAX_BINCODE_SIZE 128
  ...
  //cout << __func__ << " " << __LINE__ << ": fixed_reg_mapping =\n" << fixed_reg_mapping.fixed_reg_mapping_to_string() << endl;
  ...
  *dst_iseq_len = dst_iseq_disassemble(dst_iseq, bincode, bincode_len, NULL, dst_nextpcs, &dst_num_nextpcs, I386_AS_MODE_32);
...
/*#if ARCH_DST == ARCH_I386
  ...
  fixed_reg_mapping = fixed_reg_mapping.fixed_reg_mapping_rename_using_regmap(dst_regmap);
...
etfg_input_exec_get_arg_names_for_function(input_exec_t const *in, string const &function_name)
{
  ...
  for (const auto &e : t->get_argument_regs().get_map()) {
    ret.push_back(expr_string(e.second));
  }
  return ret;
}
...
src_iseq_get_preprocessed_tfg(etfg_insn_t const *etfg_iseq, long etfg_iseq_len, regset_t const *live_out, size_t num_live_out, map<pc, map<string_ref, expr_ref>> const &return_reg_map, context *ctx, sym_exec &se)
{
  ...
  shared_ptr<tfg_llvm_t> tfg_ret = make_shared<tfg_llvm_t>("etfg_iseq", ctx);
  tfg_ret->set_start_state(start_state);
  shared_ptr<tfg_node> start_node = make_shared<tfg_node>(pc::start());
  ...
  for (long i = 0; i < etfg_iseq_len; i++) {
    ...
    for (const auto &ei : etfg_insn->m_edges) {
      ...
      if (tfg_ret->find_node(from_pc) == 0) {
        shared_ptr<tfg_node> from_node = make_shared<tfg_node>(from_pc);
      ...
      if (tfg_ret->find_node(to_pc) == 0) {
        shared_ptr<tfg_node> to_node = make_shared<tfg_node>(to_pc);
  ...
  tfg_ret->tfg_preprocess(false, sorted_bbl_indices);
...
etfg_ret_insn_replace_return_value_with_constant(etfg_insn_t *insn, int retval)
{
  ...
  etfg_insn_edge_t &ie = insn->m_edges.at(0);
  //shared_ptr<tfg_edge const> &te = ie.get_tfg_edge();
  shared_ptr<state const> const &start_state = insn->etfg_insn_get_start_state();
  ...
  std::function<void (pc const&, state&)> f = [ctx, retval, &start_state](pc const& p, state& st)
  {
    ...
    expr_ref e = st.get_expr(oss1.str(), *start_state);
    map<expr_id_t, pair<expr_ref, expr_ref>> submap;
...
"}}}\n";
  ...
  long ret = src_iseq_from_string(NULL, insns, insns_size, etfg_ret_insn_str, I386_AS_MODE_32);
  ASSERT(ret <= insns_size);
  ASSERT(ret == 1);
...
etfg_iseq_all_start_states_are_equal(etfg_insn_t const *iseq, long iseq_len)
{
  ...
  ASSERT(iseq_len > 0);
  shared_ptr<state const> const &start_state = iseq[0].etfg_insn_get_start_state();
...
etfg_insn_t::insn_matches_template(etfg_insn_t const &templ, src_iseq_match_renamings_t &src_iseq_match_renamings/*regmap_t &regmap, imm_vt_map_t &imm_vt_map, nextpc_map_t &nextpc_map*/, bool var) const
{
  ...
  //nextpc_map_init(&nextpc_map);
  ASSERT(var);
  bool ret = etfg_insn_matches_template(&templ, this, renamings, "", var);
  if (!ret) {
    ASSERT(renamings.size() == 0);
    return false;
  }
...
bool
etfg_insn_edge_t::etfg_insn_edge_stores_top_level_op_in_reg(exreg_group_id_t g, reg_identifier_t const &r, vector<expr::operation_kind> const &ops, state const &start_state) const
{
  state const &st = m_tfg_edge->get_to_state();
  expr_ref e = st.get_expr_value(start_state, reg_type_exvreg, g, r.reg_identifier_get_id());
  context *ctx = e->get_context();
...
bool
etfg_insn_t::etfg_insn_stores_top_level_op_in_reg(exreg_group_id_t g, reg_identifier_t const &r,
    vector<expr::operation_kind> const &ops) const
{
  for (const auto &e : m_edges) {
    if (!e.etfg_insn_edge_stores_top_level_op_in_reg(g, r, ops, *m_start_state)) {
      return false;
    }
  }
  return true;
...
static bool
etfg_iseq_stores_top_level_op_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r, vector<expr::operation_kind> const &ops)
{
  for (size_t i = 0; i < iseq_len; i++) {
    if (iseq[i].etfg_insn_stores_top_level_op_in_reg(g, r, ops)) {
      return true;
    }
  }
  return false;
...
bool
etfg_iseq_stores_eq_comparison_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r)
{
  return etfg_iseq_stores_top_level_op_in_reg(iseq, iseq_len, g, r, {expr::OP_EQ, expr::OP_NOT/*XXX: need to introduce OP_NE*/});
}

bool
etfg_iseq_stores_ult_comparison_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r)
{
  return etfg_iseq_stores_top_level_op_in_reg(iseq, iseq_len, g, r, {expr::OP_BVULT, expr::OP_BVUGE});
}

bool
etfg_iseq_stores_slt_comparison_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r)
{
  return etfg_iseq_stores_top_level_op_in_reg(iseq, iseq_len, g, r, {expr::OP_BVSLT, expr::OP_BVSGE});
}

bool
etfg_iseq_stores_ugt_comparison_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r)
{
  return etfg_iseq_stores_top_level_op_in_reg(iseq, iseq_len, g, r, {expr::OP_BVUGT, expr::OP_BVULE});
}

bool
etfg_iseq_stores_sgt_comparison_in_reg(etfg_insn_t const *iseq, long iseq_len, exreg_group_id_t g, reg_identifier_t const &r)
{
  return etfg_iseq_stores_top_level_op_in_reg(iseq, iseq_len, g, r, {expr::OP_BVSGT, expr::OP_BVSLE});
}
