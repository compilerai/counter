...
class cg_with_inductive_preds : public corr_graph
{
private:
  //map<pcpair, unordered_set<predicate>> m_inductive_preds;
public:
  cg_with_inductive_preds(corr_graph const &cg/*, map<pcpair, unordered_set<predicate>> const& inductive_preds*/) : corr_graph(cg)//, m_inductive_preds(inductive_preds)
  { }

  cg_with_inductive_preds(cg_with_inductive_preds const& other) : corr_graph(other)//, m_inductive_preds(other.m_inductive_preds)
  { }
...
private:
  expr_ref cg_get_simplified_edgecond(shared_ptr<corr_graph_edge const> const& e) const;

  bool cg_outgoing_edges_at_pc_are_well_formed(pcpair const& pp) const;
  bool check_inductive_preds_on_edge(shared_ptr<corr_graph_edge const> const& e_in) const;
