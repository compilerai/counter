  ...
  if (db.is_open()) {
    ...
    while(more) {
      ...
      if (cmn_insn_t::uses_serialization()) {
        ...
        iseq_len = cmn_iseq_from_string(0, iseq, 2, insn.c_str(), I386_AS_MODE_32);
      }
      ...
      cmn_add(iseq, db.tellg(), NULL/*tfg*/);
    ...
    memset_t *memdef)
{
  ...
  vector<insn_db_match_t<cmn_insn_t, pair<streampos, shared_ptr<tfg const>>>> matched = cmn_m_db.insn_db_get_all_matches(*insn, true);
  ...
  for (const auto &m : matched) {
    ...
    shared_ptr<tfg> tfg = this->cmn_insn_get_tfg_from_insn_db_match(*insn, m, 0, g_ctx);
...
shared_ptr<tfg const>
sym_exec::cmn_get_tfg_from_insn_db_list_elem(insn_db_list_elem_t<cmn_insn_t, pair<streampos, shared_ptr<tfg const>>> &e, string const &db_file, state const &base_state)
{
  consts_struct_t const &cs = g_ctx->get_consts_struct();
  if (e.v.second) {
    return e.v.second;
  ...
  shared_ptr<tfg> tfg_var = make_shared<tfg_asm_t>("dst", g_ctx/*, cmn_m_base_state*/);
  ...
  return const_pointer_cast<tfg const>(tfg_var);
}
...
shared_ptr<tfg>
sym_exec::cmn_insn_get_tfg_from_insn_db_match(cmn_insn_t const &insn, insn_db_match_t<cmn_insn_t, pair<streampos, shared_ptr<tfg const>>> const &m,  long cur_index, context *ctx)
{
    ...
    if (state::key_represents_exreg_key(key, group, reg_id)) {
      ...
      if (!mapping.reg_identifier_is_unassigned()) {
        ...
        if (mapping.reg_identifier_denotes_constant()) {
          ...
          to = cmn_m_base_state.get_expr_value(cmn_m_base_state, reg_type_exvreg, group, mapping.reg_identifier_get_id());
        }
  ...
  list<shared_ptr<tfg_edge const>> exiting_edges;
  ...
  list<shared_ptr<tfg_edge const>> edges, new_edges;
  tfg_ret->get_edges(edges);
  for (auto e : edges) {
    DYN_DEBUG(peep_tab, cout << "before reorder e->get_to_state(): " << e->get_to_state().to_string() << endl << "regmap:\n" << regmap_to_string(&regmap, as1, sizeof as1) << endl);
    ...
    shared_ptr<tfg_edge const> new_e = e->tfg_edge_update_state(f);
    new_edges.push_back(new_e);
    DYN_DEBUG(peep_tab, cout << "after reorder new_e->get_to_state(): " << new_e->get_to_state().to_string() << endl);
  }
  ...
  for (auto e : exiting_edges) {
    ...
    tfg_ret->add_node(make_shared<tfg_node>(e->get_to_pc()));
  }
...
shared_ptr<tfg>
sym_exec::cmn_get_tfg_full(cmn_insn_t const iseq[], long iseq_len, context* ctx/*,
    bool src, bool use_memaccess_comments*/)
{
  consts_struct_t const &cs = ctx->get_consts_struct();
  shared_ptr<tfg> tfg_ret;
  tfg_ret = make_shared<tfg_asm_t>("dst", ctx/*, cmn_m_base_state*/);
  tfg_ret->set_start_state(cmn_m_base_state);

  for (long i = 0; i < iseq_len; i++) {
    shared_ptr<tfg> itfg = cmn_insn_get_tfg(&iseq[i], i, ctx);
  ...
  shared_ptr<tfg> ret = make_shared<tfg_asm_t>(*tfg_ret); //so all redundant fields (e.g., m_outoing_edges_lookup, etc.) get filled up
  assert(ret);
  //delete tfg_ret;
...
shared_ptr<tfg>
sym_exec::cmn_get_tfg(cmn_insn_t const iseq[], long iseq_len, context *ctx/*,
    //struct locals_info_t *locals_info,
    bool src, bool all_insns_are_labels, bool use_memaccess_comments*/)
{
  shared_ptr<tfg> tfg;

  assert(iseq_len > 0);

  tfg = cmn_get_tfg_full(iseq, iseq_len, ctx/*, src, use_memaccess_comments*/);
...
void
sym_exec::cmn_add(cmn_insn_t* iseq, streampos off, shared_ptr<tfg const> const &tfg)
{
