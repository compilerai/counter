...
main(int argc, char **argv)
{
  ...
  ts_init = new typestate_t[num_states];
  ASSERT(ts_init);
  //ts_fin = (typestate_t *)malloc(NUM_FP_STATES * sizeof(typestate_t));
  ts_fin = new typestate_t[num_states];
  ASSERT(ts_fin);

  src_iseq_compute_typestate_using_transmaps(ts_init, ts_fin, src_iseq, src_iseq_len, live_out, &in_tmap, out_tmaps, num_live_out, mem_live_out, *rand_states);

#if ARCH_SRC == ARCH_ETFG
  //etfg insns typically involve ADDR types, but DST insns typically involve INTADDR types; eliminate this gap by converting as follows
  ...
  emit_types_to_file(stdout, ts_init, ts_fin, num_states);
  //free(ts_init);
  delete [] ts_init;
  //free(ts_fin);
  delete [] ts_fin;
