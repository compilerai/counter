...
long
cmn_iseq_eliminate_nops(cmn_insn_t *iseq, long iseq_len)
{
  //src_tag_t pcrel_tag;
  long map[iseq_len];
  //uint64_t pcrel_val;
...
long
cmn_iseq_canonicalize(cmn_insn_t *iseq, long iseq_len)
{
  long i;
  for (i = 0; i < iseq_len; i++) {
    cmn_insn_canonicalize(&iseq[i]);
  }
  iseq_len = cmn_iseq_eliminate_nops(iseq, iseq_len);
    ...
    regmap_t *st_regmap)
{
  ...
  iseq_len = cmn_iseq_canonicalize(iseq, iseq_len);
    ...
    long max_num_iseq_vars, bool canonicalize_imms)
{
  ...
  cmn_iseq_copy(iseq_var[0], iseq, iseq_len);
  iseq_var_len[0] = cmn_iseq_canonicalize(iseq_var[0], iseq_len);
...
cmn_iseq_disassemble_with_bin_offsets(cmn_insn_t *cmn_iseq,
    ...
    long *num_nextpcs, src_ulong *bin_offsets,
    i386_as_mode_t i386_as_mode)
{
  ...
  for (binptr = bincode, cmn_insn = &cmn_iseq[0], i = 0;
    ...
    ASSERT(size > 0);
    DBG(I386_DISAS, "Disassembling %p: %hhx %hhx %hhx %hhx %hhx %hhx. size=%d\n",
    ...
    size_t *num_live_out, bool *mem_live_out, long linenum)
{
  ...
  *cmn_iseq_len = cmn_iseq_from_string(NULL, cmn_iseq, cmn_iseq_size, cmn_assembly, I386_AS_MODE_32);
  if (*cmn_iseq_len < 0) {
