...
#define gas_assert assert

bool optimize_imms_and_disps = true;
...
/*enum flag_code {
	...
	CODE_16BIT,
	CODE_64BIT };*/
enum flag_code flag_code = CODE_32BIT;
bool regular_assembly_only = false;
...
static const pseudo_typeS md_pseudo_table[] =
{
...
i386_md_begin (void)
{
...

  {
	...
	if (optab->name == NULL
	    || strcmp (optab->name, (optab - 1)->name) != 0)
	  {
	    ...
	    hash_err = gas_hash_insert (op_hash,
				    (optab - 1)->name,
				    (void *) core_optab);
	    if (hash_err)
	      {
  ...
  reg_hash = gas_hash_new ();
  {
    ...
    for (regtab = i386_regtab; regtab_size--; regtab++)
      {
	hash_err = gas_hash_insert (reg_hash, regtab->reg_name, (void *) regtab);
	if (hash_err)
	  as_fatal (_("can't hash %s: %s"),
		    regtab->reg_name,
		    hash_err);
...
{
  gas_hash_print_statistics (file, "i386 opcode", op_hash);
  gas_hash_print_statistics (file, "i386 register", reg_hash);
}

#ifdef DEBUG386

/* Debugging routines for i386_md_assemble.  */
...
int
i386_md_assemble (char *line, char *output)
{
  unsigned int j;
  char mnemonic[MAX_MNEM_SIZE];
  const insn_template *t;

  gas_assemble_mode = ARCH_I386;
  frag_grow (256); //sorav
  DBG2(TCDBG_I386, "i386_md_assemble entry: intel_syntax=%d, flag_code=%d, line=%s\n", intel_syntax, flag_code, line);
  /* Initialize globals.  */
  memset (&i, '\0', sizeof (i));
  ...
  DBG2(TCDBG_I386, "line = '%s'", line);
  line = parse_insn (line, mnemonic);
  if (line == NULL) {
    DBG(TCDBG_I386, "%s", "returning -1\n");
    return -1;
  }

  DBG2(TCDBG_I386, "parsing operands: line='%s'\n", line);
  ...
  DBG3(TCDBG_I386, "i.types[0]=%s.\n", hex_string((uint8_t*)&i.types[0], sizeof(i.types[0]), as1, sizeof as1));

  if (optimize_imms_and_disps) {
  if (i.imm_operands)
    optimize_imm ();
...
*/
  ...
  /* sorav */
  fragS *orig_frag = frag_now;
  char *cur = obstack_next_free(&frchain_now->frch_obstack);
  /* We are ready to output the insn.  */
  output_insn ();

  //ASSERT(frag_now == orig_frag);
  char *end = obstack_next_free(&frchain_now->frch_obstack);
  memcpy(output, cur, end - cur);
  return end - cur;
}
...
parse_insn (char *line, char *mnemonic)
{
  ...
  while (1)
      ...
      if (token_start == l)
	{
      ...
      current_templates = (const templates *) gas_hash_find (op_hash, mnemonic);
...
check_suffix:
	...
	case QWORD_MNEM_SUFFIX:
	  i.suffix = mnem_p[-1];
	  mnem_p[-1] = '\0';
	  current_templates = (const templates *) gas_hash_find (op_hash,
                                                             mnemonic);
	  break;
  ...
  for (t = current_templates->start; t < current_templates->end; t++)
    {
      ...
      if (regular_assembly_only && !t->has_regular_assembly) {
        continue;
      }
...
process_operands (void)
{
  ...
  if (i.tm.opcode_modifier.sse2avx && i.tm.opcode_modifier.vexvvvv)
      ...
      else if (i.tm.opcode_modifier.implicit1stxmm0)
	{
	  ...
	  i.op[0].regs
	    = (const reg_entry *) gas_hash_find (reg_hash, "xmm0");
	  i.types[0] = regxmm;
...
duplicate:
  ...
  /* If a segment was explicitly specified, and the specified segment
     ...
     always be used.  */
  if ((i.seg[0]) && (i.seg[0] != default_seg || regular_assembly_only))
    {
	  ...
	  /* RIP addressing for 64bit mode.  */
		...
		i.rex |= REX_B;
		 ...
		 complications.  */
		  ...
		  default_seg = &ss;
	      if (regular_assembly_only && i.disp_operands == 0)
		{ //sorav
		  fake_zero_displacement = 1;
		  if (i.tm.operand_types [op].bitfield.vec_disp8)
		    i.types[op].bitfield.vec_disp8 = 1;
...
i386_immediate (char *imm_start)
{
  ...
  DBG3(TCDBG_I386, "input_line_pointer = %s; exp->X_add_number=%lx\n", input_line_pointer, (unsigned long)exp->X_add_number);
  bool is_symbolic_constant = false; //sorav
  SKIP_WHITESPACE ();
...
i386_index_check (const char *operand_string)
{
  ...
  if (current_templates->start->opcode_modifier.isstring
      ...
      if (current_templates->start->opcode_modifier.w)
	{
	  ...
	  expected_reg = gas_hash_find (reg_hash,
				    di_si[addr_mode][type.bitfield.esseg]);

	}
      else
	expected_reg = gas_hash_find (reg_hash, bx[addr_mode]);
      ...
      if (*base_string == ')')
	{
	  ...
	  if (*base_string == ','
	      ...
	      if (*base_string == ',')
		{
		  ...
		  else /* if (*base_string == REGISTER_PREFIX) commented by sorav */
		    {
...
void
i386_md_convert_frag (bfd *abfd ATTRIBUTE_UNUSED, segT sec ATTRIBUTE_UNUSED,
                 fragS *fragP)
{
  unsigned char *opcode;
...
void
i386_md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
...
parse_real_register (char *reg_string, char **end_op)
{
  ...
  *end_op = s;
  ...
  r = (const reg_entry *) gas_hash_find (reg_hash, reg_name_given);
  ...
  if (r == i386_regtab /* %st is first entry of table  */)
      ...
      if (*s == '(')
	{
	  ...
	  if (*s >= '0' && *s <= '7')
		...
		{
		  *end_op = s + 1;
		  r = (const reg_entry *) gas_hash_find (reg_hash, "st(0)");
		  know (r);
		  return r + fpr;
		}
	    }
...
void
md_operand_dummy (expressionS *e)
{
  char *end;
  const reg_entry *r;
