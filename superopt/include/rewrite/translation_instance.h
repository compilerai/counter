...
public:
  ...
  bool pc_is_etfg_function_return(src_ulong pc, string &function_name) const;
  bool pc_is_etfg_function_callsite(src_ulong pc, vector<string> &argnames) const;
...
#if ARCH_SRC == ARCH_ETFG
  map<unsigned, dshared_ptr<state const>> m_function_id_to_input_state_map;
#endif
public:
...
void etfg_read_input_file(input_exec_t *in, std::map<std::string, dshared_ptr<eqspace::tfg>> const &function_tfg_map);
...
void input_exec_init(input_exec_t *in);
